(function (app) {
  ...
  function init() {
    ...
    for (var index = 1; index <= ContentArray.length; index++) {
      ...
      headerContent +=
        '<div class="myPage ' +
        pageFirstSection +
        '" id="pageFirstSection_' +
        index +
        '"><div class="para_ tabindex Cont_Que">' +
        ContentArray[index - 1].question +
        '</div><button id="AnswerDiv_' +
        index +
        '" class="AnswerDiv" type="button" isSelected="false"><span class="tabindex show">Show answer</span></button><div class="textArea" id="textA' +
        index +
        '"><div id="wrap' +
        index +
        '" class="wrapper AnswerDiv_para"><div class="contentHolder tabindex Cont_Ans">' +
        ContentArray[index - 1].answer +
        '</div><div id="scroller"></div></div></div></div>';
    }
  ...
  function AnswerDivFun(e) {
    ...
    if ($textArea.is(":visible")) {
      ...
      $textArea.attr("hidden", "hidden").attr("aria-hidden", "true"); //new addedd
    } else {
      ...
      $textArea.removeAttr("hidden"); // new added
      $textArea.attr("aria-hidden", "false"); // new added
      $textArea.slideDown("fast", function () {
        // Animation complete.
      ...
      if (
        $("#wrap" + indexId).prop("scrollHeight") >
        $("#wrap" + indexId).height()
      ) // check scroll
      {
        //fnAddScroll(indexId);
